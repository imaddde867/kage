...
cd "$KAGE_DIR"
echo "  ⟳ Loading Qwen3.5-9B (TurboQuant KV)..."
...
llm = get_local_llm()
llm.load()
ratio = llm._cache.compression_ratio if llm._cache else 0
print(f"  ✓ Qwen3.5-9B loaded. TurboQuant KV active (ratio: {ratio:.2f}x)")
PYEOF
else
...
# Kill second-brain when Kage exits
trap 'echo ""; echo "  ⏹ Stopping second-brain (pid $SECOND_BRAIN_PID)..."; kill "$SECOND_BRAIN_PID" 2>/dev/null; wait "$SECOND_BRAIN_PID" 2>/dev/null; echo "  Done."' EXIT
